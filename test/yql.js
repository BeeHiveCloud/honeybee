...
String.prototype.toInt = function () {
  ...
  str = str.trim() str = str.replace( /\,/g, '' );
...
var query = y.xpath( rawresult, "//table[@class='yfnc_tabledata1']/tr/td/table/tr");
  ...
  if ( query.*.length() !== 0 ) {
    var y = 0; var x = 0; var numcolumns = 0;
    var data_array = new Array();
    // Read Column Headings
    data_array[0] = new Array() data_array[0][0] = "Headings"
    for each ( var th in query[0].td) {
      data_array[x] = new Array(); data_array[x][y] = th.*.text().toString();
      outputdata.appendChild(<column x={x} y={y}>{data_array[x][y]}</column>); x++;
    }
    numcolumns = x;
    // Read Rows
    y = 1; var i = 1;
      ...
      if(row.td.length() >= numcolumns){
        ...
        for each (var column in row.td){
          if("spacer" in column) {}
            ...
            outputdata.appendChild(<row x={x} y={y}>{dataitem}</row>); x++;
          }
        }
        y++;
      } i++;
    }
    var numrows = y; x = 0; y = 0;
...
// Return statsdata strucuture response.object = outputdata;
